    ...
    sideModalFun:function(options1){
        ...
        if($(".designer-mask").length==0){
            ...
            let rightModel='<div class="designer_detail">'+
                '<div class="detail_head"><span class="headTitle"></span><a class="headClose"><img src='+ctxPath+'/img/close.png></a></div>'+
                '<div class="detail_context"><img style="width:50px;margin:100px auto;display: block;" src='+ctxPath+'/img/loading.gif></div>'+
                '</div>';
            $(body).append(rightModel);
        }
        $(".designer_detail .headClose").mouseenter(function(){
            $(this).children("img").attr("src",ctxPath+'/img/closeH.png');
        }).mouseleave(function(){
            $(this).children("img").attr("src",ctxPath+'/img/close.png')
        });
        ...
        //按钮Html
        let operateDiv='<div class="detail-footer" style="padding:30px 32px;position:absolute;bottom:0px;left:0;right:0"><button type="button" class="btn btn-bcloud-blue active btn-lg btn-confirm" style="float:left;width: 100px;">提交</button>'+
        ...
        function rightHide(){
            ...
            $mask.hide();
            setTimeout(function(){
                $rightModel.find(".detail_context").html('<img style="width:24px;margin:100px auto;display: block;" src='+ctxPath+'/img/loading.gif>');
            },200);
        }
        //添加内容文本
